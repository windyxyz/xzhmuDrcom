...
`drcom-xzhmu.sh` 是一个独立的 OpenWrt/BusyBox `ash` 脚本，用于在路由器上完成徐医 DrCOM 校园网登录、状态检查、保活和注销。它同步浏览器扩展 1.1.0 经生产抓包验证的登录与终端解绑逻辑，但不依赖 Chrome、Node.js、curl、jq 或 Python。
...
- 会话文件通过字段白名单读取，不会作为 shell 脚本执行；保存时先写入私有临时文件再原子替换。会话仍只包含 IP、MAC 与时间。
