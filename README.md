...
- 注销优先从 `chkstatus` 读取当前在线账号与 `ss4` MAC；缺少有效 `ss4` 时只用该完整在线账号查询一次 `find_mac`，并支持学校真实响应中的 `list[].online_mac`。取得有效 MAC 后先请求 `unbind_mac`，未能确认离线则回退完整 `Portal/logout`，并按 300、800、1500 毫秒复核；接口返回成功但状态仍为在线或未知时不会伪造离线。
